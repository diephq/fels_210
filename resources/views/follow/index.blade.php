...
@section('content')
    <div class="container category">
        <div class="col-md-8 col-md-offset-2">
            <h1 class="page-header">{{ trans('message.follow.all_users') }}</h1>
        </div>
        <div class="row">
            <div class="col-md-8 col-md-offset-2">
                <ul class="list-group">
                    @foreach($users as $user)
                        ...
                        <li class="list-group-item">
                            ...
                            @if (!empty($user->following[0]))
                                {{ Form::submit(trans('message.follow.following'), ['class' => 'btn-success follow', 'id' => 'follow', 'target_id' => $user->id ]) }}
                            @else
                                {{ Form::submit(trans('message.follow.follow'), ['class' => 'btn-success follow', 'id' => 'follow', 'target_id' => $user->id]) }}
                            @endif
                        </li>
                    @endforeach
                </ul>
